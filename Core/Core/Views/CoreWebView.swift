            ...
            document.querySelectorAll('iframe').forEach(iframe => {
                ...
                if (/\\/(courses|accounts)\\/[^\\/]+\\/external_tools\\/retrieve/.test(iframe.src)) {
                    ...
                    if (match.length == 2) {
                        ...
                        video.src = 'https://lms.flexidata.vn/users/self/media_download?entryId='+mediaID+'&media_type=video&redirect=1'
                        video.setAttribute('poster', 'https://lms.flexidata.vn/media_objects/'+mediaID+'/thumbnail?width=550&height=448')
